...
def saturated_vp_water(water_temp):
    result = np.empty(0)
    for x in water_temp:
        val = .611 * math.exp((17.27 * x) / (237.3 + x))
        result = np.append(result, [val])
    return result
...
def latent_heat_vapor(water_temp):
    ...
    for i in range(len(water_temp)):
        val = 1000000 * (2.501 - (.002361 * water_temp[i]))
        result = np.append(result, [val])
    return result
...
def saturation_vp_slope(air_temp, e_s):
    ...
    for i in range(len(e_s)):
        val = (4100 * e_s[i]) / ((237 + air_temp[i]) ** 2)
        result = np.append(result, [val])
    return result
...
def penman_evap_eq(s, shortwave, longwave, rho_water, l_e, psy_constant, c_air, rho_air, e_s, e_a, r_a):
    ...
    for i in range(len(shortwave)):
        val = (((s[i] * (shortwave[i] + longwave[i])) / (rho_water * l_e[i] * (s[i] + psy_constant)))) + ((c_air * rho_air * psy_constant * (e_s[i] - e_a[i])) / (rho_water * l_e[i] * r_a[i] * (s[i] + psy_constant)))
        result = np.append(result, [val])
    return result
...
def case1_latent_heat_flux(rho_water, l_e, penman_evap):
    ...
    for i in range(len(l_e)):
        val = (-rho_water) * l_e[i] * penman_evap[i]
        result = np.append(result, [val])
    return result
...
def case2_latent_heat_flux(rho_water, l_e, mass_transfer_evap):
    result = np.empty(0)
    for i in range(len(l_e)):
        val = -rho_water * l_e[i] * mass_transfer_evap[i]
        result = np.append(result, [val])
    return result