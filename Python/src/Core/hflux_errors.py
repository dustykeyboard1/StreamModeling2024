...
def handle_errors(*args):
    ...
    if time_mod.ndim != 2 or time_mod.shape[0] != 1:
        raise TypeError("Time_mod must be a row vector.")
    if dist_mod.ndim != 2 or dist_mod.shape[0] != 1:
        raise TypeError("Dist_mod must be a row vector.")
    if time_temp.ndim != 2 or time_temp.shape[0] != 1:
        raise TypeError("Time_temp must be a row vector.")
    if dist_temp.ndim != 2 or dist_temp.shape[0] != 1:
        raise TypeError("Dist_temp must be a row vector.")

    if not isinstance(temp_mod, np.ndarray): 
        raise TypeError("Temp_mod must be a numpy array.")
    if not isinstance(temp, np.ndarray):
        raise TypeError("Temp must be a numpy array.")
