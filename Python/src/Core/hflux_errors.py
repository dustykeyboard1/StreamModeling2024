...
def handle_errors(*args):
    ...
    if not unattend: 
        ...
        plt.imshow(residuals, aspect='auto', cmap='jet', origin='lower', extent=[0, 1400, 0, 1400])
        ...
        plt.plot(dist_temp, np.mean(temp, axis = 1), '--ko', linewidth=1.5, markerfacecolor='b', markersize=8)
