...
def hflux(input_data):
    ...
    match method:
        case 1:
            ...
            filename = os.path.join(os.getcwd(), "Python", "Results", "CSVs")
            heat_flux_file = heat_flux / 60
            heat_flux_file.tofile(os.path.join(filename, "heatflux_data.csv"), sep=',')
            sol_refl.tofile(os.path.join(filename, "solar_refl_data.csv"), sep=',')
            longwave.tofile(os.path.join(filename, "long_data.csv"), sep=',')
            atm.tofile(os.path.join(filename, "atmflux_data.csv"), sep=',')
            land.tofile(os.path.join(filename, "landflux_data.csv"), sep=',')
            back.tofile(os.path.join(filename, "backrad_data.csv"), sep=',')
            latent.tofile(os.path.join(filename, "evap_data.csv"), sep=',')
            sensible.tofile(os.path.join(filename, "sensible_data.csv"), sep=',')
            bed.tofile(os.path.join(filename, "conduction_data.csv"), sep=',')
            shortwave.tofile(os.path.join(filename, "solarflux_data.csv"), sep=',')
