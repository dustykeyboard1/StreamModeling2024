...
def main():
    ...
    output = open(
        os.path.join(output_location, "csv_diff.txt"), "w"
    )  # file to write to

    for mat in os.listdir(matlab):
        for py in os.listdir(python):
            if (
                os.path.splitext(mat)[1] == ".csv"
                and os.path.splitext(py)[1] == ".csv"
                and mat == py
            ):  # only compare them if they're csv files and have the same name
                compare_files(
                    os.path.join(matlab, mat), os.path.join(python, py), output, py
                )
    ...
    if len(mat) != len(py):
        if len(mat) > len(py):
            output.write(
                ...
                + " has "
                + str(len(mat) - len(py))
                + " more entries than python output"
            )
        else:
            output.write(
                "python output "
